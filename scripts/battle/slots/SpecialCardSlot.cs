...
public partial class SpecialCardSlot : CardSlot<SpecialCard, SpecialCardData>, IActivable
{
    ...
    [Export] private Texture2D _magicActivatedTexture;
    [Export] private Texture2D _trapActivatedTexture;
	...
	public void SetPopupPosition() {
	    // Popup.Center(GetOffset());
	    Popup.SetOffset(GetOffset() * GetSprite().Scale);
	    // Popup.AlignLeft(GetSize().X);
	    // Popup.AlignBottom(GetSize().Y);
	}
	
	public void OnClicked(Node viewport, InputEvent @event, int shapeIdx) {
	    if (@event is InputEventMouseButton mouseEvent
		&& mouseEvent.ButtonIndex == MouseButton.Left) {
		if (this is IActivable activable) {
		    if (activable.CanActivate()) {
			activable.ShowActivationPopup(this);
		    }
		    else {
	...
	private void RefreshVisual()
	{
		...
		case SpecialType.Magic:
		    GetSprite().Texture = _magicActivatedTexture;
		    break;
		default:
		    break;
		}
